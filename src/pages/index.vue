  ...
  <div class="w-full h-full bg-slate-950">
    <div class="h-full w-600px overflow-auto mx-auto py-32px box-border">
        ...
        <div class="w-full h-fit box-border p-4 bg-slate-100/15  rounded-3xl">
          ...
          <div class="flex flex-col gap-4">
            <div class="w-full grid grid-cols-2 justify-between">
              ...
              <button @click="()=>rootSelectClick(RootSelectType.LT)"
                      class="h-48px hover:text-blue-500 hover:border-blue-500 transition cursor-pointer rounded-xl text-slate-200 text-xl bg-transparent border border-3px border-solid border-slate-300">
                点击选择
              </button>
            </div>
            <div class="w-full grid grid-cols-2 justify-between">
              <span class="text-slate-200">监控区域右下角</span>
              <button @click="()=>rootSelectClick(RootSelectType.RB)"
                      class="h-48px hover:text-blue-500 hover:border-blue-500 transition cursor-pointer rounded-xl text-slate-200 text-xl bg-transparent border border-3px border-solid border-slate-300">
                点击选择
              </button>
            </div>
      ...
      <div class="w-full space-y-4 mt-40px">
        <div class="w-full p-4 bg-slate-100/15 rounded-3xl box-border flex items-center justify-around gap-4"
             v-for="member of teamMember" :key="member.id">
          <div>
            <span class="text-zinc-200">队员 {{ member.id }}</span>
          </div>
          <div class="grow flex">
            <button @click="()=>onClickTeamMemberSelect(member.id)"
                    class="h-48px grow hover:text-blue-500 hover:border-blue-500 transition cursor-pointer rounded-xl text-slate-200 text-xl bg-transparent border border-3px border-solid border-slate-300">
              点击选择
            </button>
            <button @click="() => removeTeamMember(member.id)"
                    class="h-48px w-48px aspect-square bg-transparent border-none text-slate-200">
      ...
      <div class="w-full h-fit mt-4">
        <button class="bg-none w-full h-48px rounded-xl bg-blue-600 text-zinc-200 border-none cursor-pointer"
                @click="addTeamMember">
          添加队员
        </button>
      </div>
